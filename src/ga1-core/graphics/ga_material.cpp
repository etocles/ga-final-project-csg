...
void ga_csg_material::bind(const ga_mat4f& view_proj, const ga_mat4f& transform)
{
	...
	ga_uniform select_uniform = _program->get_uniform("u_selected");
	ga_uniform secondary_uniform = _program->get_uniform("u_is_secondary");
	...
	select_uniform.set(_selected);
	secondary_uniform.set(_secondary);
